...
DATADIR=$1
snapstrs=(033 058 078 151)
zstrs=(4.0 2.0 1.0 0.0)
snapnums=(33 58 78 151)
...
do
	...
	bash skid_one_m25n256.sh ${snapnums[snapi]} ${zstrs[snapi]} $line
	# 3. so
    ...
    # 4. GSMFs and SMHMs
    echo gsmf_smhm.sh
    bash gsmf_smhm_m25n256.sh
    # 5. Metal Rho-T Diagram
    echo mrhotz.sh $line
    bash mrhotz.sh $line
    echo mzr.sh $line
    bash mzr.sh $line
	# 6. Move stuff and prepare for export
