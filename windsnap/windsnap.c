...
int GridCount(float x, float y, float z, float m, float delayt, float meto)
{
  int ix, iy, i;
  if(z > Zmin && z < Zmax){
	...
	return 0;
	  ...
	  Nwind ++;
	  Ntot ++;
	  return 1; // A wind based on delayt
	}
      }
    }
  }
...
void WriteGrid(char *outgridfilename, char *outwindfilename)
{
  FILE *Fout;
  int ix, iy, i;
  double vdotr;
  ...
  for(i = 0; i < header.nsph; i++){
    if(gp[i].hsmooth < 0){ /* It's marked as wind. */
...
{
  /* Calling Sequence: */
  /* windsnap binfile, auxfile, modelname */
  ...
  for(i=0; i<header.nsph; i++){
    ...
    if(GridCount(gp[i].pos[0], gp[i].pos[1], gp[i].pos[2],
		 gp[i].mass, aux_gp[i].delaytime, aux_gp[i].metal[1])) /* C, O, Si, Fe; */
      { gp[i].hsmooth *= -1; } // Marked as wind
  }
  
  fprintf(stdout, "Grid Calculated: Nwind/Ntot = %d/%d\n", Nwind, Ntot);

  WriteGrid(outgridfilename, outwindfilename);
  free(nodes_array);
}
