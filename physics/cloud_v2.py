...
USE_MINIMUM_EXPANSION_SPEED = True # v_shock - 3.0 c_s ?
ADJUST_RHOC_WITH_PEVAP = True
KHI_MODIFIED_TIMESCALE = True # Modified tau_kh by exp(Rc/lambda_kh)
...
class study_case():
    ...
    def cloud_shock(self):
        ...
        print "v_shock_approx = %5.3f; log(rho_c/rho_c0) = %5.3f", self.v_shock_approx / 1.e5, log(self.rho_c/self.rho_c0)*self.cs_c / 1.e5
        # self.v_max = log(self.rho_c/self.rho_c0)
    ...
    def dynamical_update(self, dt):
        ...
        mlr_tail = 0.871e-15 * self.T_a ** 2.5 / self.r_c
        ...
        if(conduction.evaluate_saturation_point(self.T_c, n_ps, T_ps, self.r_c) < 0):
            ...
            mlr = 5.30e-15 * T_ps ** 2.5 * self.L / 3.5
            T_star = 1.e4
        else:
            T_star = conduction.solve_for_saturation_point(n_ps, T_ps, self.r_c, verbose=False)
            mlr = 5.22e-19 * n_ps * sqrt(T_star) * self.r_c * self.L / 3.5
        ...
        if(KHI_MODIFIED_TIMESCALE):
            lkh = conduction.lambda_kh(1.0, self.n_c/n_ps, T_ps, self.n_c, fkhi=10.0, fcond=self.f_cond)
        ...
        if(flux_cond <= 1.5*flux_mass and self.L < 1500.*ac.pc): # Only expand when evaporation is inefficient     
            self.L_prev = self.L
            ...
            if(self.L_prev < self.L):
                print "Lmax reached: t= %5.1f, L= %5.2f, v_max = %3.1f, flux_ratio = %3.1f" % \
                    (self.time/self.tau_cc, self.L/ac.pc, self.v_max, flux_cond/flux_mass)
                print flux_cond, flux_mass
                self.L_prev = self.L
