...
from numpy import exp, log, pi, sqrt, logspace
...
class study_case():
    ...
    def derived_quantities(self, verbose=False):
        self.rho_c = self.n_c * 1.30 * pc.mh
    ...
    def dynamical_update(self, dt, verbose=False, conduction_flag=True, compression_flag=False, geometry="spherical"):
        ...
        print mlr / (conduction.mass_loss_rate(self.r_c, n_ps, T_ps) * 0.5)
        ...
        if(compression_flag == False):
            ...
            self.r_c = sqrt(self.M_c / (self.N_c * pc.mh * 1.30) / pi)        
            if(self.L > L_eq):
                self.L = L_eq
        else:
...
params = parameters(kind=0)

M_0 = 6.7e4 * ac.msolar
def special_case(cond = True):
    v_h = 800. # [km/s]
    T_eq = 1.e4
    ...
    ttot = 0.0
    tx, r, mc, v = [], [], [], []
    rc25, L25, dv25 = -1.0, -1.0, -1.0
    rc50, L50, dv50 = -1.0, -1.0, -1.0    
    while ttot < END_T:
    ...
    print "t50 = %5.3f tau_cc; (Rc = %5.3f pc, L = %5.3f pc, dv = %5.3f km/s)" % (t50, rc50, L50, dv50)
    print "t25 = %5.3f tau_cc; (Rc = %5.3f pc, L = %5.3f pc, dv = %5.3f km/s)" % (t25, rc25, L25, dv25)
    plt.plot(tx, v, "r-")
    plt.plot(tx, mc, "b-")
    plt.show()
