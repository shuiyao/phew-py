...
/* **************** WARNING on the Tmax **************** */
/* Update: SH200110. From Now On, use tmax from snapshot. */
...
void main(int argc, char **argv)
{
  ...
  sprintf(binname, "%s/snapshot_%s.bin", basename, snapstr);
  sprintf(auxname, "%s/snapshot_%s.aux", basename, snapstr);
  sprintf(idnumname, "%s/snapshot_%s.idnum", basename, snapstr);
